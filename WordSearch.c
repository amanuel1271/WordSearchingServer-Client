...
/*int main(int argc,char *argv[]){
   ...
   char progname[1024];
   unsigned int ID = 1;
   char search[1024],word[2048];

   Word_list = calloc(DATA_SIZE,sizeof(struct word)); // free this at the end by calling some
   ...
   if (dir == NULL){
    ...
    while( (dent=readdir(dir)) != NULL)
    {
        ...
        manage_words(&fp,&Word_list,dent->d_name,&size_of_array,&ID);
    ...
    // Search for 10 words then the program will deallocate all allocated memory and then exit
    for(int j = 0;j < 10;j++){
        printf("%s> ",progname+2);
        fflush(stdout);
        ...
        //Search_array(Word_list,size_of_array,word);
	    //printf("\n");
        Memset_to_zero(word,2048);
        Memset_to_zero(search,1024);
    }

    deallocate_all_mem(Word_list,size_of_array);
    closedir(dir);
    return 0;
 }*/
